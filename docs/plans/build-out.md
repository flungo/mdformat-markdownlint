...
- [ ] Upstream filings: an empty-cell fix and the task-list escape that breaks a link whose text is `x` to mdformat-gfm; a comment-adjacency option, an exclusion hook and, if it proves worth filing, a way for a plugin to stop a run with a message rather than a traceback proposed to mdformat; the plugin shrinks as each lands.
