...
- [ ] Plugin behaviours, each with its corpus entries: derived `number` and `compact_tables`, from an explicit MD029 or MD060 style alone ([ADR-005](../decisions/005-derive-options-only-from-explicit-settings.md)), with a list starting at zero numbered on its own and a table whose padded form would cross MD013's limit written compact on its own, where MD060 allows it; the markdownlint configuration read as markdownlint-cli2 reads it, `extends`, the YAML form and per-directory merging included, and the JavaScript forms refused; `ignores` matched with markdownlint-cli2's globby semantics, bare directory names, braces and negations included, and `overrides` applied with the same matching, which the configuration reader carries unapplied until then; comment blocks kept adjacent; single-space empty compact cells; `[//]: #` definitions preserved; `.mdformat.toml` validated against the derived options; refusal of unsatisfiable settings, of which MD029 `zero`, MD060 `tight` and `aligned_delimiter` with `compact` are refused with the derivation.
...
- [ ] Upstream filings: an empty-cell fix and the task-list escape that breaks a link whose text is `x` to mdformat-gfm; a start number above one accepted by MD029's `ordered` and `one_or_ordered` styles proposed to markdownlint, since a list starting at 2 or above is a finding before and after any formatter and mdformat cannot change a list's start without changing what it renders; a comment-adjacency option, an exclusion hook, consecutive numbering of a list that starts at zero and, if it proves worth filing, a way for a plugin to stop a run with a message rather than a traceback proposed to mdformat; the plugin shrinks as each lands.
