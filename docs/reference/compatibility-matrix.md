...
The plugin sets two of these from the configuration markdownlint-cli2 would lint the file with, read as that tool reads it ([how the plugin reads the configuration](configuration.md)), and only from a setting that names what mdformat is to write, so that at markdownlint's defaults mdformat's own decisions stand ([ADR-005](../decisions/005-derive-options-only-from-explicit-settings.md)); the rest it leaves to mdformat's own configuration.

| markdownlint setting | mdformat option | Derivation | Evidence |
| -- | -- | -- | -- |
| `MD029.style` | `number` | `one` gives `false` and `ordered` `true`; `one_or_ordered` and the default derive nothing, as does the rule disabled, so mdformat's own configuration stands; `zero` is refused, being what mdformat never writes, and so is a value the rule does not know | [Corpus for `ordered`](../../tests/corpus/md029-ordered/case.toml) and [for `zero`](../../tests/corpus/md029-zero/case.toml), [unit tests](../../tests/test_derived.py) for `one` and the unknown value |
| `MD060.style` | `compact_tables` | `compact` gives `true`; `aligned` gives `false`; `any` and the default leave mdformat's own setting in place, as does the rule disabled; `tight` is refused, being what mdformat never writes, as are `aligned_delimiter` with `compact` and a value the rule does not know | [Corpus for `compact`](../../tests/corpus/md060-compact/case.toml), [for `tight`](../../tests/corpus/md060-tight/case.toml) and [for `aligned_delimiter` with `compact`](../../tests/corpus/md060-compact-aligned-delimiter/case.toml), [unit tests](../../tests/test_derived.py) for `aligned` and the unknown value |
| `MD013` with `tables` true, the default | none | A padded table lengthens every row to its widest cell, which is the one way mdformat's own style breaks a line-length limit, but compact tables everywhere is not what the rule asks; the plugin instead writes compact only a table that would cross the limit, where MD060 allows it, a rendering behaviour rather than an option ([ADR-005](../decisions/005-derive-options-only-from-explicit-settings.md)) | Pending |
...
| MD029 ol-prefix | `one` | Guaranteed | [Corpus](../../tests/corpus/md029-one/case.toml) | At mdformat's default, every item after the first is written as `1.`; the plugin also derives `number` as `false`, so a `number = true` in mdformat's own configuration cannot contradict the rule, which the unit tests prove since the harness passes mdformat no option. A list whose first item is not `1.` or `0.` violates the rule before and after formatting |
| MD029 ol-prefix | `one_or_ordered`, the default | Guaranteed | [Corpus](../../tests/corpus/md029-one-or-ordered/case.toml) | The style accepts what mdformat writes, every item after the first as `1.`, and the plugin derives nothing ([ADR-005](../decisions/005-derive-options-only-from-explicit-settings.md)). A list starting at `0.` is the next row. A list starting at 2 or above violates the rule before and after formatting |
| MD029 ol-prefix | `one_or_ordered`, the default, with a list starting at `0.` | Bridged | Pending | mdformat alone renders it `0. 1. 1.`, which the rule reads as ordered from zero and rejects on the third item; the plugin numbers that list alone, `0. 1. 2.`, and leaves every other list as mdformat writes it ([ADR-005](../decisions/005-derive-options-only-from-explicit-settings.md)) |
| MD029 ol-prefix | `ordered` | Bridged | [Corpus](../../tests/corpus/md029-ordered/case.toml) | `number` derived as `true`, so a list is written 1, 2, 3, and 0, 1, 2 from zero; mdformat alone writes every item after the first as `1.`, a finding on each. A list of ten or more items is zero-padded to `01.`, which the rule accepts. A list starting at 2 or above violates the rule before and after formatting |
| MD029 ol-prefix | `zero` | Unsatisfiable | [Corpus](../../tests/corpus/md029-zero/case.toml) | Items after the first are never `0.`; the plugin refuses the setting |
...
| MD013 line-length | default, with `tables` true and a table whose padded form is wider than the limit, MD060 not `aligned` | Bridged | Pending | mdformat pads every row to the widest cell, so a table under the limit as written can cross it after formatting, and the rule reports every row that does; the plugin writes that table alone compact, and leaves every other table padded ([ADR-005](../decisions/005-derive-options-only-from-explicit-settings.md)). MD060 at `compact` derives compact tables and the row does not arise |
| MD013 line-length | default, with `tables` true and a table whose padded form is wider than the limit, MD060 `aligned` | Unsatisfiable | Probe | Padding is what the configuration asks for, so the rows the rule reports are the author's to shorten, or `tables` false is the setting |
...
| MD040 fenced-code-language | any, on an indented code block | Unsatisfiable | Probe | An indented block is outside the rule's scope; mdformat rewrites it as a fence with no language, which puts it inside, and only the author knows the language. The one place a document that lints clean at markdownlint's defaults stops doing so after formatting; ADR-002 names it. The corpus pins the behaviour as a side effect: the `md014` and `md046` cases list MD040 as incidental on every document with an indented block, since the fence mdformat writes for it reports the rule after formatting |
...
| MD060 table-column-style | `aligned` | Guaranteed | [Corpus](../../tests/corpus/md060-aligned/case.toml) | At mdformat's default, every cell is padded to its column's widest; the plugin also derives `compact_tables` as `false`, so a compact setting in mdformat's own configuration cannot contradict the rule, which the unit tests prove since the harness passes mdformat no option |
| MD060 table-column-style | `compact` | Bridged | [Corpus for the derivation](../../tests/corpus/md060-compact/case.toml); prototype for the empty cell | `compact_tables` derived as `true`, so every pipe has one space each side; mdformat alone pads every cell, a finding on each padded pipe. An empty cell is rendered as above, pending, and no document of the case has one until it lands. The delimiter row is `--`, or `:-:` and `-:` for aligned columns, which the style accepts |
| MD060 table-column-style | `tight` | Unsatisfiable | [Corpus](../../tests/corpus/md060-tight/case.toml) | Every pipe is padded with a space; the plugin refuses the setting |
...
| MD060 table-column-style | `aligned_delimiter` true with `compact` | Unsatisfiable | [Corpus](../../tests/corpus/md060-compact-aligned-delimiter/case.toml) | The compact delimiter row is `--` regardless of the header; the plugin refuses the setting |
...
The settings the plugin refuses to run under, collected from the rows above; the MD029 and MD060 entries are refused today, with a value of either rule's `style` the plugin does not know, and the rest is pending:
