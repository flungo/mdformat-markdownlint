...
| Path | Purpose |
| -- | -- |
| `tests/corpus/<case>/` | One directory per case, named for what its documents share, which is the configuration they run under: `baseline` for markdownlint's defaults, `preset` for the config package alone, and for a rule the preset with that rule the subject, `md018` at the preset's own settings, `md003-atx` for the rule's `style` at that value, `md025-level-2` for any other option at a value, `md041-allow-preamble` for a boolean at true and `md001-front-matter-title-empty` for a string emptied. A construct that is the exception to the rule's row under the same configuration is a document of that case with its own status, never a case named for the construct |
