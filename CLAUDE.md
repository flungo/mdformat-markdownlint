...
- The preset ships as the npm package `markdownlint-config-mdformat` from the directory of that name, the plugin as `mdformat-markdownlint` on PyPI from the root, versioned independently; neither depends on the other, the corpus tests the pair, and the plugin-only and preset-only adopters are both real ([ADR-003](docs/decisions/003-ship-the-preset-as-an-npm-package.md)).
- The YAML forms of the configuration are read with a loader built on ruamel.yaml, which mdformat-frontmatter already brings in, that resolves and constructs only the YAML 1.2 core schema, as js-yaml does for markdownlint-cli2 ([ADR-004](docs/decisions/004-read-yaml-configuration-with-ruamel-under-the-core-schema.md)).
- The plugin derives an mdformat option only from a markdownlint setting that names what mdformat is to write, MD029 `one` or `ordered` and MD060 `compact` or `aligned`, and nothing at a rule's default, so at markdownlint's defaults mdformat's own decisions stand ([ADR-005](docs/decisions/005-derive-options-only-from-explicit-settings.md)).
